# Pragati - An EdTech App
## Airtel IQ Hackathon

<p align="center">
  <img width="450px" height="330px" src="https://github.com/Marcresi/pragati/assets/101006060/27e45b06-60b2-42ef-9b36-6f3c1e84c26f"/>

<br />
## Problem 
In order to realise the goal of demographic divide, every citizen of the country needs to be educated for job, be it a scientist, a doctor, engineer or whatever to meet not only the needs of our society but also the other societies elsewhere in the world. Every student aspires to become a great person and self reliant, but their passion and likings are diverse. 

Also to cater to the aspirations of the students and people looking for new opportunities and facilitate them to aim high, they are required to be informed and suggested about the number and type of jobs this country has in the future years and also the number of business persons, entrepreneurs, workforce in all areas of employment. 
<br />
## User Segments 

Students/Graduates :  The chatbot(Pragati) will suggest them their career goals and prospects, this can be what they are best in and according to that what is the best career(livelihood) they can choose for themselves. This can drastically reduce in making of wrong career choices.

Parents  : The benefit of using a chatbot is that a parent can tell what is right for their child and get suggestions on ways to select their learning curves based on their child’s interests.

Working Professionals: Pragati will provide easier access of resourceful information to enhance their skills and improve their knowledge.
<br />
## Solution
This project focuses on personalized career guidance and resources for students, graduates, and working professionals in the education sector. Using data analysis and machine learning techniques, the project recommends the best career paths based on individuals' skills, interests, and strengths.

The project utilizes curated job datasets and skill repositories, and uses natural language processing to match user input with relevant career options. It also provides working professionals with easy access to educational resources and jobs openings.

The project's versatility allows continuously learning from user interactions to improve recommendations. 

<br />
  > ## Applicaton Images.
  
  <img width="320px" height="640px" src="https://github.com/Marcresi/pragati/assets/101006060/029da282-a041-4fb5-a8db-9b15dc7719ce"/>
  <img width="320px" height="640px" src="https://github.com/Marcresi/pragati/assets/101006060/cca565a8-52e7-4137-b6ec-6b1e885e6fb6"/>
  <img width="320px" height="640px" src="https://github.com/Marcresi/pragati/assets/101006060/2eb5608c-edf5-4bc5-9965-706956bf4192"/>
  <img width="320px" height="640px" src="https://github.com/Marcresi/pragati/assets/101006060/84ace80b-4db2-46d7-8fb7-774222a892f2"/>
  <img width="320px" height="640px" src="https://github.com/Marcresi/pragati/assets/101006060/ed19554f-f0f7-43a3-b2c2-dbf4b3220470"/>
  


<br />
  > ## Solution and App demo video
  https://user-images.githubusercontent.com/77913276/227733559-89e758ba-4f6d-4e9d-b115-a109a5d2b029.mp4
  <br />
  > ## Chatbot demo video
  https://user-images.githubusercontent.com/77913276/227733663-348b81dc-9613-4874-93ec-81222fc25b28.mp4

### Try it out, It's live
The following link will redirect you to demo whatsapp number. 
https://wa.me/918904587744?text=Hi

## Technology or libraries Used

- Dart -> Flutter Framework


## Getting Started
For help getting started with Flutter, view our
[online documentation](https://flutter.dev/docs), which offers tutorials,
samples, guidance on mobile development, and a full API reference.

